...
class DashboardPage(QWidget):
    ...
    def _export_excel_legacy(self):  # WHY: keep original sync export as reference; replaced by threaded version below.
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                c = conn.cursor()
                c.execute("SELECT ad_soyad, SUM(CASE WHEN tur='Avans' THEN tutar ELSE -tutar END) FROM avans_kesinti WHERE tarih BETWEEN ? AND ? GROUP BY ad_soyad", (vals['date_from'], vals['date_to']))
                for r in c.fetchall():
                    avans_map[r[0]] = r[1]
